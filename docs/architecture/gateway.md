...
| `internal/observability/` | slog 初始化与 v1/v2 协议使用计数（`/api/status` 的 `protocol_usage`）。 |
...
WebSocket server 的实现分层：`internal/transport/wscore` 管写泵/背压/心跳（两代共用）；v2 在 `internal/protocol/pbws`（帧编解码、直通白名单、事件扇出）；v1 在 `internal/server/websocket*.go`（弃用：连接生命周期 + 字符串路由 + domain handler）。域逻辑（终端门控/响应后处理、chat 编排）在 `internal/protocol/shared` 与 `internal/chatcmd`，两代协议调用同一份实现。
